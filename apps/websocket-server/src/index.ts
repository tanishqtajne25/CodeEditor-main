...
function getRoomUsers(roomId: string) {
  return rooms[roomId].map((user: any) => ({
    id: user.userId,
    name: user.name,
    color: user.color,
  }));
...
async function process() {
  ...
  wss.on("connection", (ws, req) => {
    ...
    const presenceSnapshot = rooms[roomId]
      .filter((user: any) => user.userId !== userId && user.presence)
      .map((user: any) => ({
        userId: user.userId,
        name: user.name,
        color: user.color,
    ...
    ws.on("message", (message) => {
      ...
      if (data.type === "cursorPresence") {
        const currentUser = rooms[roomId].find((user: any) => user.userId === userId);
        if (!currentUser) {
          return;
        }
    ...
    ws.on("close", () => {
      ...
      rooms[roomId].forEach((user: any) => {
        user.ws.send(JSON.stringify({ type: "users", users }));
        user.ws.send(JSON.stringify({ type: "presenceRemove", userId }));
      });
